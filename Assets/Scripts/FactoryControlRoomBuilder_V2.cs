...
/// V2: não reconstrói nada automaticamente ao iniciar o jogo; só constrói pelo menu de contexto.
...
/// </summary>
public class FactoryControlRoomBuilder_V2 : MonoBehaviour
{
    ...
    [Header("Runtime / Segurança")]
    [Tooltip("Deixe falso. Se verdadeiro, reconstrói a sala ao iniciar. Normalmente isso NÃO deve ser usado.")]
    public bool buildRoomOnStart = false;
    [Tooltip("Fecha a porta automaticamente quando o jogo começa, sem criar sala nova.")]
    public bool closeDoorOnPlayStart = true;
    ...
    private void Awake()
    {
        ...
        // Importante: este script NÃO reconstrói a sala por padrão no Play.
        // Ele só procura a porta já gerada e controla a interação.
    }
    ...
    private void Start()
    {
        ...
        if (buildRoomOnStart)
        {
            BuildOrRebuildControlRoom();
        }
        else if (closeDoorOnPlayStart && doorPivot != null)
        {
            CloseDoorNow();
        }
    }
    ...
    {
        if (Application.isPlaying && !buildRoomOnStart)
        {
            Debug.LogWarning("FactoryControlRoomBuilder_V2: não reconstrua a sala durante o Play. Saia do Play Mode e use o menu de contexto no Editor.", this);
            return;
        }
    ...
    {
        Transform container = FindFirstTransformByNameStartsWith(ContainerName);
        if (container != null)
    ...
    [ContextMenu("Clean Generated Control Room Duplicates")]
    public void CleanGeneratedControlRoomDuplicates()
    {
        DeleteOldGeneratedRoom();
    }
    ...
    private void DeleteOldGeneratedRoom()
    {
        ...
        foreach (Transform t in all)
        {
            ...
            // Pega também cópias que a Unity renomeia como "Generated_Control_Room_From_Scratch (1)".
            if (t.name.StartsWith(ContainerName))
                toDelete.Add(t.gameObject);
        }

        // Segurança extra: se por algum motivo peças geradas ficaram soltas fora do container, remove também.
        foreach (Transform t in all)
        {
            ...
            string n = t.name;
            bool looksGeneratedPiece =
                (n.StartsWith("Control_Room_") && (n.Contains("_Generated") || n.Contains("_Frame") || n.Contains("_Support") || n.Contains("_Beam") || n.Contains("_Platform"))) ||
                n.StartsWith("Generated_ControlRoom_");

            if (looksGeneratedPiece && !toDelete.Contains(t.gameObject))
                toDelete.Add(t.gameObject);
        }
    ...
    private Transform FindFirstTransformByNameStartsWith(string prefix)
    {
        Transform[] all = GetComponentsInChildren<Transform>(true);
        foreach (Transform t in all)
        {
